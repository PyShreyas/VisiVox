...
def clear_database():
    ...
    cursor = conn.cursor()
    cursor.execute("DELETE FROM detected_text")
    conn.commit()
    conn.close()
...
def plot_perspective_analysis(data):
    ...
    plt.pie(top_counts, labels=top_unique_distortions, autopct='%1.1f%%', startangle=90,
            colors=plt.cm.rainbow(np.linspace(0, 1, len(top_counts))))
